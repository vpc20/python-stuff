...
def dfs_iterative_all_paths(maze_matrix, start, end):
    """
    Performs Depth-First Search (DFS) iteratively to find all possible paths through a maze.
    ...
    all_paths = []
    stack = [([start], set())]  # Store path and visited set in each stack element

    while stack:
        path, visited = stack.pop()
        row, col = path[-1]  # Get the last cell in the current path
        ...
        for dr, dc in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
            ...
            if 0 <= new_row < rows and 0 <= new_col < cols and (new_row, new_col) not in visited and maze_matrix[new_row][new_col] in ('.', 'E'):
                ...
                new_visited = visited.copy()  # Create a copy of visited set for each path
...
if start is None or end is None:
    ...
    all_paths = dfs_iterative_all_paths(maze_matrix, start, end)
